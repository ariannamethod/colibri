...
# Mistral (Llama-3-style) case-split pretok needs sub-classes of L plus marks M.
Ll   = ranges(lambda c: cat(c) == "Ll")           # lowercase letters
LuLt = ranges(lambda c: cat(c) in ("Lu", "Lt"))   # uppercase + titlecase
M    = ranges(lambda c: cat(c).startswith("M"))    # marks (Mn, Mc, Me)
...
emit("uni_Ll", Ll); emit("uni_LuLt", LuLt); emit("uni_M", M)
...
/* Mistral (Llama-3-style) case-split groups: upper=[Lu Lt Lm Lo M], lower=[Ll Lm Lo M] */
static inline int is_upgrp(uint32_t c){ return (is_L(c)&&!is_Ll(c))||is_M(c); }
static inline int is_logrp(uint32_t c){ return (is_L(c)&&!is_LuLt(c))||is_M(c); }
#endif""")
